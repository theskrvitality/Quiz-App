...
export class QuestionComponent implements OnInit {
  ...
  allQuestions: QuizQuestion[] = [
    ...
    },
    {
      questionId: 3,
      questionText:
        "Which of the following is the first step in setting up dependency injection?",
      options: [
  ...
  ngOnInit() {
    ...
    this.progressValue =
      (100 * (this.currentQuestion + 1)) / this.totalQuestions;
    this.countdown();
  }
  ...
  displayNextQuestion() {
    ...
    if (
      typeof document.getElementById("question") !== "undefined" &&
  ...
  increaseProgressValue() {
    this.progressValue = parseFloat(
      ((100 * (this.getQuestionID() + 1)) / this.totalQuestions).toFixed(1)
    );
  }
  ...
  calculateTotalElapsedTime(elapsedTimes) {
    return (this.completionTime = elapsedTimes.reduce(
      (acc, cur) => acc + cur,
      0
    ));
  }
